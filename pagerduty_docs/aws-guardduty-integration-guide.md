...
2. Follow the prompts and [configure the service](/main/docs/services-and-integrations#create-a-new-service) to your preferences. On the Integrations screen, select **Amazon GuardDuty** from the search bar dropdown.
...
5. Perform the following:
   ...
   * **Endpoint**: Paste your **Integration URL** (generated in steps [above](#in-pagerduty)).
   * Perform the following based on your preference:
     + **If you are integrating with Global Orchestrations**: We recommend that the **Enable raw message delivery** checkbox is **checked**.
     + **If you are *not* integrating Global Orchestrations**: Ensure that the **Enable raw message delivery** checkbox is **unchecked**.
...
7. * If you have not enabled GuardDuty, please [follow Amazon’s documentation](https://docs.aws.amazon.com/guardduty/latest/ug/guardduty_settingup.html#guardduty_enable-gd).
   * If you have already enabled GuardDuty, proceed to step 8.
8. Once you have enabled GuardDuty, you can begin building EventBridge Rules to send alerts to PagerDuty. Search and select **EventBridge** from the Services search bar.
9. Select **Rules** from the left menu, then click **Create Rule**. One or more rules can be created to send specific events to PagerDuty when a GuardDuty finding is opened.
10. On the next screen, perform the following:
...
12. On the next page, perform the following:
    ...
    * Click **Next** to continue.
13. On the next page, *optionally* add tags to your preference. Click **Next** to continue.
14. On the final page, review your settings and click **Create Rule**. If you would like to create more rules, repeat steps 9-14.

The integration is now complete. To ensure the configuration was successful, navigate back to the Amazon GuardDuty console, select **Settings** in the left menu, scroll down and click **Generate Sample Findings**, then select **Findings** in the left menu. You will see the sample findings that have been generated, and in PagerDuty you will see the correlated sample alert.
...
* [Table of Contents](#)
* + [Amazon GuardDuty + PagerDuty Benefits](#amazon-guardduty--pagerduty-benefits)
