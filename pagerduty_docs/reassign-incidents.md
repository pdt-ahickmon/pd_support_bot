...
Reassign incidents to escalation levels, escalation policies or specific users

If the assigned responder cannot respond to a PagerDuty incident, you may choose to reassign the incident to the next user on the current escalation level, to a different escalation policy, or to a specific user.

Reassign an Incident
====================

There are three methods of reassigning an incident:

* **Reassign to an Escalation Level (Recommended)**: Reassigning an incident to an [escalation level](/main/docs/escalation-policies#escalation-rules) means escalating the incident to the next user in the incident’s current escalation policy. This method is preferred because the escalation policy will continue to cycle through escalation rules until a user acknowledges the incident.
* **Reassign to an Escalation Policy (Recommended)**: Reassigning the incident to a different [escalation policy](/main/docs/escalation-policies) than the one currently being used is also ideal. The escalation policy will continue to cycle through escalation rules until a user acknowledges the incident.
* **Reassign to a User**: Reassigning the incident to an individual [user](/main/docs/users) means that only the selected user will be notified. The incident’s escalation policy will no longer be used, and other users will not be notified if the selected user does not acknowledge the incident.

> 📘 Low-Urgency Incidents
> -----------------------
> 
> You cannot reassign [low-urgency](/main/docs/configurable-service-settings#notification-urgency) PagerDuty incidents to a specific escalation level. You can, however, reassign them to an escalation policy or a user.

Reassign an Incident in the Web App
-----------------------------------

1. Click an incident’s **title** to go to its detail page and click **Reassign**.
2. Select the desired **Escalation Policy**, **User** or **Escalation Level** from their respective tabs in the dropdown.
3. Click **Reassign**.

### Reassign Multiple Incidents in the Web App

1. Navigate to the **Incidents** page.
2. Select the checkbox to the left of each incident and click **Reassign**.

![A screenshot of the PagerDuty web app showing how to reassign multiple incidents](https://files.readme.io/f5246bb-reassigning-delegating-reassign-multiple.png "reassigning-delegating-reassign-multiple.png")



Reassign multiple incidents



4. Select the desired **Escalation Policy**, **User** or **Escalation Level** from their respective tabs in the dropdown.
5. Click **Reassign**.

Reassign an Incident in the Mobile App
--------------------------------------

1. Tap the **incident** that you want to reassign.
2. Tap **More** in the bottom right and tap **Reassign**, or scroll through the incident actions in the Triage tab and tap **Reassign**.
3. Select one of the following based on your preferences:
   * **To reassign to a different escalation policy**: In the **Escalation Policies** tab, select your preferred **escalation policy**.
   * **To reassign to the next user on the escalation level**: In the **Users** tab, select a user under **This Policy**.
   * **To reassign to a different user**: In the **Users** tab, select a user under **All Users**.
4. Tap **Reassign** to confirm.

### Reassign Multiple Incidents in the Mobile App

1. On the **Open Incidents** screen, tap the  in the top right and select **Select Incidents**.
2. Select the **radio buttons** to the left of each incident you would like to reassign, tap the  again and select **Reassign**.
3. Select one of the following based on your preferences:
   * **To reassign to a different escalation policy**: In the **Escalation Policies** tab, select your preferred **escalation policy**.
   * **To reassign to the next user on the escalation level**: In the **Users** tab, select a **user** under **This Policy**.
   * **To reassign to a different user**: In the **Users** tab, select a **user** under **All Users**.
...
Reassign an Incident Via API
----------------------------

To learn more about reassigning incidents via API, please visit our [Manage Incidents API Documentation](https://developer.pagerduty.com/api-reference/b3A6Mjc0ODEzOQ-manage-incidents).

Escalate an Incident
====================

Escalation assigns the incident to another user on the incident's current escalation policy. You may select the next user from the [escalation level](/main/docs/escalation-policies#escalation-levels) that you are on, or a user on a different escalation level. Escalating is the same action as [reassigning to an escalation level](#reassign-an-incident), but this action is also available for quick and easy use under the **Escalate** button on PagerDuty incidents.

> 📘 Low-Urgency Incidents
> -----------------------
> 
> You cannot escalate [low-urgency](/main/docs/configurable-service-settings#notification-urgency) PagerDuty incidents.

Escalate an Incident in the Web App
-----------------------------------

1. Click an incident’s **Title** to go to its detail page and then click **Escalate**.
2. Select the desired **user** from the dropdown and the incident will automatically escalate to that user.

Escalate an Incident in the Mobile App
--------------------------------------

1. Tap the **incident** that you want to escalate.
2. Tap **More** in the bottom right and tap **Escalate**, or scroll through the incident actions in the Triage tab and tap **Escalate**.
3. Select the desired **user** from the list and then tap **Escalate** in the confirmation dialog that appears.
...
![A screenshot of a PagerDuty SMS notification showing how to escalate an incident](https://files.readme.io/001f33b-image2.png "Escalate an incident via SMS")
...
> ------------------
> 
> The response numbers to acknowledge, resolve and escalate incidents via SMS may change depending on the number of incidents in your account and any notifications you have already received. Check the SMS closely for the correct response numbers for your incidents, as the response number used in the example screenshot may not be valid for your incidents.

Escalate an Incident Via API
----------------------------

To learn more about escalating incidents via API, please visit our [Manage Incidents API Documentation](https://developer.pagerduty.com/api-reference/f79f697c07f6c-manage-incidents).
...
* [Table of Contents](#)
* + [Reassign an Incident](#reassign-an-incident)
    - [Reassign an Incident in the Web App](#reassign-an-incident-in-the-web-app)
    - [Reassign an Incident in the Mobile App](#reassign-an-incident-in-the-mobile-app)
    - [Reassign an Incident Via API](#reassign-an-incident-via-api)
  + [Escalate an Incident](#escalate-an-incident)
    - [Escalate an Incident in the Web App](#escalate-an-incident-in-the-web-app)
    - [Escalate an Incident in the Mobile App](#escalate-an-incident-in-the-mobile-app)
    - [Escalate an Incident via SMS](#escalate-an-incident-via-sms)
    - [Escalate an Incident Via API](#escalate-an-incident-via-api)
