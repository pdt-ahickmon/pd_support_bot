...
* [import\_users.rb](https://github.com/PagerDuty/public-support-scripts/blob/master/import_users/import_users.rb)

You will need the following:

* A [general access REST API key](/main/docs/api-access-keys#generate-a-general-access-rest-api-key).
...
* There is no need to include a header in the file, and the title and Teams fields are optional. If the user is on multiple [Teams](/main/docs/teams), separate each Team with a semicolon.
* The script will create SMS, phone call, and email [notification rules](/main/docs/user-profile#notification-rules) using the email address and phone number provided. The notification rules will notify users immediately when they are assigned to an incident, however users can adjust these settings later on their [User Profile](/main/docs/user-profile#notification-rules).
* The user's role can be `admin`, `user`, `limited_user`, `observer`, `restricted_access`, `read_only_user` (Full Stakeholder), or `read_only_limited_user` (Limited Stakeholder). If a role is not specified, the script defaults to `user`. To see how these values map to values on the webpage, please refer to our section on [Roles in the REST API and SAML](/main/docs/advanced-permissions#roles-in-the-rest-api-and-saml). You can find additional information on user roles and permissions in the following articles:
  + [User Roles](/main/docs/user-roles)
  + [Advanced Permissions](/main/docs/advanced-permissions)
...
After populating a spreadsheet in this format, export it as a spreadsheet file (e.g., `.CSV`, `.XLSX`, `.NUMBERS`, etc.) and have your Account Owner [contact our Support Team](mailto:support@pagerduty.com) to request help importing the users.
...
| **Role** (*Required*) | A valid REST API user role (`admin`, `user`, `limited_user, observer`, `restricted_access`, `read_only_user`, `read_only_limited_user`). **Note**: `read_only_user`and `read_only_limited_user` roles require Stakeholder user licenses. Read [Billing, Invoices and Payments](/main/docs/billing-invoices-payments#stakeholder-users) for more information about Stakeholder users. |
| **Title** | Job title. |
| **Country Code** | 1-3 digit country calling code. Please refer to [Supported Countries](/main/docs/supported-countries) for more information about valid country codes. |
| **Phone Number** | The user’s phone number where they will receive SMS and phone call notifications. |
| **Teams** | [Teams](/main/docs/teams) that the user should be added to. This can include multiple Teams separated by semicolons (`;`). If the Team does not exist, it will be created when the script runs. |
| **Team Role** | User role that will be applied to the given Teams. Must be a valid [Team role](/main/docs/advanced-permissions#team-roles) (i.e., Responder, Observer, Manager). |
