...
PagerDuty can notify users via push notification when various incident lifecycle events occur.
...
| A user is added as a [responder](/main/docs/add-responders) to a *new* incident | “PagerDuty `[incident number]` (`[user]`): Please help with "[`[incident number]`] `[incident title]`" |
| A user is added as a [responder](/main/docs/add-responders) to an *existing* incident | "`[user]` requested your response. `[custom message]`" |
...
| A user is about to go on call ([On-Call Handoff Notification](/main/docs/escalation-policies#on-call-handoff-notification-settings)) | "Went on call on `[date and time]` for escalation policy `[escalation policy]` (Level `X` - `[schedule name]`)." |
...
![Push notification](https://files.readme.io/795bb84-push-notification.png "Push notification")
...
* + [Push Notification Content](#push-notification-content)
    - [Push Notification Example](#push-notification-example)
