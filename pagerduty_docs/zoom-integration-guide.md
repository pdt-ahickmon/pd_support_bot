...
* Incident Workflows can create on-demand Zoom meetings and attach them to incidents.
...
PagerDuty users with a Responder role or higher can create Zoom meetings via incidents.

How it Works
============

* On a PagerDuty incident, responders can create an incident-specific Zoom ID by [clicking a link](#create-a-zoom-meeting-manually) or they can optionally run a pre-configured [Incident Workflows](/main/docs/incident-workflows). This information is sent as an event to AWS Lambda, which transforms it to a Zoom-compatible format and it creates a meeting ID. **Note**: The meeting ID is incident-specific and cannot be used for other incidents.
* Once the meeting ID has been created, this information is sent back through the same lambda to PagerDuty and creates a [Conference Bridge](/main/docs/conference-bridge) with the meeting ID information.
* Responders can join the Zoom conference bridge directly from the incident in the web app and mobile app, via conference bridge notifications, or they can use [One Touch to Join](/main/docs/conference-bridge#one-touch-to-join-conference-bridg).
...
1. Navigate to the [Zoom App Marketplace](https://marketplace.zoom.us/) and install the **PagerDuty** app.

> 🚧 EU Service Region
> -------------------
> 
> If your PagerDuty account is in the EU [Service Region](/main/docs/service-regions), please install the **PagerDuty EU** app in the Zoom App Martketplace.

2. You will see the following authorization screen. Click **Allow**.

![Allow PagerDuty app](https://files.readme.io/c0e4e86-image1.png "Allow PagerDuty app")
...
3. If prompted, sign into your PagerDuty account. Please read [Log In to PagerDuty](/main/docs/log-in-to-pagerduty) for more information about logging in to your PagerDuty account.
4. After successfully installing the integration, you should see a confirmation message in your browser.

![A screenshot indicating that Zoom is connected to PagerDuty](https://files.readme.io/c326768-image3.png "Connected to PagerDuty")



Connected to PagerDuty
...
1. In PagerDuty, click an incident’s **title** to view the incident details screen.
2. Next to **Conference Zoom**, click **Create Zoom Meeting**. The new meeting ID will automatically populate.
3. Responders can now click this meeting link from the incident in the web app, mobile app or via conference bridge notifications and [One Touch to Join](/main/docs/conference-bridge#one-touch-to-join-conference-bridge).

Create a Zoom Meeting via Incident Workflows
--------------------------------------------

Follow the [Incident Workflows guide](/main/docs/incident-workflows#create-an-incident-workflow-from-scratch) and select the [Create a Zoom Meeting](/actions/docs/create-a-zoom-meeting) action. After publishing, you can then create a Zoom meeting via the Incident Workflow.

Enable Zoom Settings for Optimal Use
------------------------------------
...
If the account where the Zoom meeting ID is created does not have **Join before host** enabled for both the **Account** and **Group** level, other users will not be able to join the meeting until this administrator joins as the host. Please read [Zoom's documentation on how to enable **Join before host**](https://support.zoom.com/hc/en/article?id=zm_kb&sysparm_article=KB0060501) on both the **Account** level and the **Group** level.
...
2. Search and select the **PagerDuty** app. Select **Manage** in the left side panel, then click **Remove** next to the Remove the app section.
...
At this time, this integration supports linking one Zoom account with one PagerDuty account.

Why don’t I see a telephone number attached to the PagerDuty incident for my Zoom meeting?
------------------------------------------------------------------------------------------

If your Zoom account does not provide telephone number dial-in, only the URL will be attached to the PagerDuty incident. **Note**: Zoom removed telephone dial-in for free Zoom accounts, please check your Zoom account pricing plan.
...
* [Table of Contents](#)
* + [Zoom + PagerDuty Integration Benefits](#zoom--pagerduty-integration-benefits)
  ...
  + [Zoom User Guide](#zoom-user-guide)
    - [Create a Zoom Meeting Manually](#create-a-zoom-meeting-manually)
    - [Create a Zoom Meeting via Incident Workflows](#create-a-zoom-meeting-via-incident-workflows)
    - [Enable Zoom Settings for Optimal Use](#enable-zoom-settings-for-optimal-use)
  + [Uninstall the Zoom Integration](#uninstall-the-zoom-integration)
  + [FAQ](#faq)
    - [Can I link more than one PagerDuty account to the same Zoom account?](#can-i-link-more-than-one-pagerduty-account-to-the-same-zoom-account)
    - [Why don’t I see a telephone number attached to the PagerDuty incident for my Zoom meeting?](#why-dont-i-see-a-telephone-number-attached-to-the-pagerduty-incident-for-my-zoom-meeting)
