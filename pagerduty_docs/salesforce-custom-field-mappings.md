...
The Salesforce Custom Field Mappings feature allows you to define field-level mapping between Salesforce fields and PagerDuty incident [Custom Fields](https://support.pagerduty.com/main/docs/custom-fields-on-incidents) of the same data type. When you map fields, the options for single and Multi-Select fields and other standard values will sync bidirectionally. You can then [link a PagerDuty incident to a Salesforce record](https://support.pagerduty.com/main/docs/salesforce-service-cloud-user-guide#create-or-link-incidents), allowing custom field data to sync bidirectionally.

Custom Fields defined as part of the integration will count towards the account’s limits for custom fields. For more information, please see our [Custom Fields on Incidents](https://support.pagerduty.com/main/docs/custom-fields-on-incidents) article.

> 📘 Availability
> --------------
> 
> Salesforce Custom Field Mappings are available on the Enterprise Customer Service pricing plan. Please contact our [Sales Team](https://www.pagerduty.com/contact-us/#contact-sales) to upgrade to an account plan with this feature.
...
> * Salesforce Custom Fields Mappings is only available from the [Salesforce Service Cloud integration version v3.19](/main/docs/salesforce-service-cloud-integration-guide#upgrade-to-v319) .

Activate Custom Field Mappings
==============================

1. In Salesforce, navigate to the **PagerDuty Configuration** page.
2. Select the **Custom Field Mappings** tab.
3. Click the **Go to PagerDuty** button to start mapping Salesforce fields with PagerDuty custom fields.
...
1. In PagerDuty, navigate to **Integrations**  **Extensions**  click **Edit** on the **Salesforce** extension tile  **Select Accounts**  **Custom Fields Mapping**.
2. Click **New Mapping** in the top right.
3. **Select a Salesforce field** (Required): Select your preferred **Salesforce field** from the dropdown. We will only fetch Salesforce fields that match the approved field types.
4. There are two options for creating a custom field mapping:
   1. **Map an Existing Custom Field**: Select the existing **Custom Field** that you’d like to map to the selected Salesforce field. **Note**: We will only show PagerDuty fields that are of a compatible field type to the Salesforce field.
   2. **Create and Map a New Custom Field**: Click **New PagerDuty Custom Field** to create a new PagerDuty field to map the Salesforce field. Enter the following information:
      1. **Display Name**: This will be prefilled with the Salesforce field name; change as needed.
      2. **Field Name**: This will be prefilled with the Salesforce field name; change as needed.
      3. **Description** (Optional): Enter an optional description.
      4. The **field type** will be auto-selected based on the selected salesforce field.
...
> ----------------
> 
> You must delete the Custom Field mapping (using instructions below) before [deleting the Custom Field on the Custom Fields Page](https://support.pagerduty.com/main/docs/custom-fields-on-incidents#delete-a-custom-field).

1. Navigate to **Integrations**  **Extensions**  click **Edit** on the **Salesforce** extension tile  **Select Accounts**  **Custom Fields Mapping**.
...
In order for custom field data to sync bidirectionally, you must link the PagerDuty incident to a Salesforce record. Please see our [Salesforce Service Cloud User Guide](https://support.pagerduty.com/main/docs/salesforce-service-cloud-user-guide#create-or-link-incidents) for more information.
...
* [Table of Contents](#)
* + [Activate Custom Field Mappings](#activate-custom-field-mappings)
  + [Create Custom Field Mappings](#create-custom-field-mappings)
    - [Precedence](#precedence)
    - [View Custom Field Mappings](#view-custom-field-mappings)
    - [Edit Custom Field Mappings](#edit-custom-field-mappings)
    - [Delete Custom Field Mappings](#delete-custom-field-mappings)
  + [Salesforce Fields Eligible for Mapping](#salesforce-fields-eligible-for-mapping)
  + [Post-mapping Behavior](#post-mapping-behavior)
    - [Post-mapping Behavior on the Custom Fields Mapping Page](#post-mapping-behavior-on-the-custom-fields-mapping-page)
    - [Post-mapping Behavior on the Custom Fields Page](#post-mapping-behavior-on-the-custom-fields-page)
    - [Other Post-mapping Behavior](#other-post-mapping-behavior)
  + [Sync Custom Field Data Bidirectionally](#sync-custom-field-data-bidirectionally)
