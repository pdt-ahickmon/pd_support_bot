...
* [A Service Was Disabled or in a Maintenance Window](#a-service-was-disabled-or-in-a-maintenance-window)
* [Event Orchestration is Configured to Suppress Certain Alerts](#event-orchestration-is-configured-to-suppress-certain-alerts)
* [Email Integration Filters](#email-integration-filters)
* [Email Management Rules Were Appending Triggers to Existing Incidents](#section-email-management-rules-were-appending-triggers-to-existing-incidents)
* [No One Was On-call](#section-no-one-was-on-call)

A Service Was Disabled or in a Maintenance Window
=================================================

When a service is [disabled](/main/docs/maintenance-windows#disable-a-service) or in a [Maintenance Window](/main/docs/maintenance-windows), new incidents will not trigger or be created on that service. Because an incident is not created, PagerDuty will not send notifications to the service's on-call responders.

Event Orchestration is Configured to Suppress Certain Alerts
============================================================

If you have [Event Orchestration](/main/docs/event-orchestration) configured on your account, you may have rules that are set to suppress alerts that meet certain criteria. In those cases, an incident will not be triggered.

Navigate to **AIOps**  **Event Orchestration** and select the **orchestration** that contains the action rule that you would like to check. While reviewing which actions are taken, check **Incident Data**  **Basic Event** to see if the checkbox **Suppress incident and notifications** is enabled.

Email Integration Filters
=========================

If you have regex filters set up on your email integration service, check whether your email filters may be [filtering out emails](/main/docs/email-management-filters-and-rules#email-filters) that you want to trigger incidents.
...
![Email integration settings](https://files.readme.io/7948e76-why-incidents-fail-regex-rules.png "why-incidents-fail-regex-rules.png")
...
* [Regex Examples](/main/docs/regular-expressions#regex-examples)
...
Email integration settings have email management rule settings, which are separate from [email filters](/main/docs/email-management-filters-and-rules#email-filters). While email filters determine which emails trigger incidents for your service, incident creation settings determine how emails create new incidents.
...
...then an incident was most likely appended to an existing incident, and could explain why a new incident was not created. When a trigger event is appended to an incident it will appear in the incident's timeline. [Learn more about incident creation settings](/main/docs/email-management-filters-and-rules#trigger-and-resolve-alertsincidents).
...
For example, if your escalation policy only has the following schedule associated with it, where one user is on-call from 00:00 - 08:00, if a trigger event comes in between 08:01 - 23:59, PagerDuty will not create an incident no one will be on-call during that time.

To address this, check the [Escalation Policy](/main/docs/escalation-policies) associated with your service and make sure that someone will be on-call when you need incidents to trigger.

> 📘 Trigger an Incident in the Web App
> ------------------------------------
> 
> If you try to trigger an incident through the [web app](/main/docs/incidents#trigger-an-incident-via-web-app) while no one is on-call on the escalation policy associated with that service, you will receive the error **Incident could not be assigned**.
...
* + [A Service Was Disabled or in a Maintenance Window](#a-service-was-disabled-or-in-a-maintenance-window)
  + [Event Orchestration is Configured to Suppress Certain Alerts](#event-orchestration-is-configured-to-suppress-certain-alerts)
  + [Email Integration Filters](#email-integration-filters)
  + [Email Management Rules Were Appending Triggers to Existing Incidents](#email-management-rules-were-appending-triggers-to-existing-incidents)
  + [No One Was On-call](#no-one-was-on-call)
